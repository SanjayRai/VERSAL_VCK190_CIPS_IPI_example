...
void fpga_PROGRAM_NORTH_PR_WITH_CLEARING (fpga_xdma_dev_mem *fpga_AXI_Lite_ptr, string PR_binFile_name, string PR_Clearing_binFile_name) {
    ...
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_GPIO_PR_ISOLATION, ISOLATE_NORTH_PR); 
    cout << "Programing Partial binfile name = " << PR_Clearing_binFile_name << endl;
    ICAP_prog_PR_binfile (fpga_AXI_Lite_ptr, PR_Clearing_binFile_name);
    cout << "Programing Partial binfile name = " << PR_binFile_name << endl;
    ICAP_prog_PR_binfile (fpga_AXI_Lite_ptr, PR_binFile_name);
    fpga_AXI_Lite_ptr->fpga_poke(AXI_LITE_GPIO_PR_ISOLATION, DEISOLATE_NORTH_PR); 
}
...
/* Program Partial Bit file */
    ...
    cout << "PR Bitfile size =  " << dec << file_size << " Bytes \n";
    ...
    for (int i = 0 ; i < file_size_words; i++) {
        ...
        if (((i+1) % 60) == 0) {
            // Write to the COntrol register to drain the Data FIFO every 60 or writes 
            dbg_counter++;
            //cout << "ICap DBG Counter = " << dec << dbg_counter << endl; 
            //usleep(10);
            fpga_AXI_Lite_ptr->fpga_poke(ICAP_CNTRL_REG, 0x1); 
            itn_count = 0;
    ...
    //SRAI_dbg_wait("ICAP_prog_PR_binfile PRE FInal Flush");
    // Final ICAP Fifo Flush
    fpga_AXI_Lite_ptr->fpga_poke(ICAP_CNTRL_REG, 0x1); 
    itn_count = 0;
    ret_data = 0;
    ...
    //SRAI_dbg_wait("ICAP_prog_PR_binfile POST FInal Flush");
